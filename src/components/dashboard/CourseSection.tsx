    ...
    <section className="px-4 py-4 md:px-8 md:py-6">
      ...
      <div className="flex items-center justify-between mb-3 md:mb-4">
        ...
        <button
          ...
          className="flex items-center gap-1 text-purple-400 hover:text-purple-300 transition-colors text-xs md:text-sm flex-shrink-0 ml-2"
        >
      ...
      <div className="flex gap-3 md:gap-4 overflow-x-auto pb-2 scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0">
        {filteredModules.map((module, index) => {
          ...
          return (
            ...
            >
              {/* Thumbnail */}
              <div className="relative aspect-video rounded-lg md:rounded-xl overflow-hidden mb-2 md:mb-3 bg-zinc-800">
                {module.thumbnail ? (
                  <img
                  ...
                  <div className="w-full h-full bg-gradient-to-br from-purple-900/40 to-zinc-800 flex items-center justify-center">
                    <span className="text-3xl md:text-4xl font-bold text-purple-400/50">
                      {index + 1}
                    </span>
                  </div>
                )}
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  <div className="w-10 h-10 md:w-14 md:h-14 rounded-full bg-white/90 flex items-center justify-center">
                    <Play className="w-4 h-4 md:w-6 md:h-6 text-black fill-current ml-0.5" />
                  </div>
                </div>
                ...
                {/* Badge */}
                {isCompleted && (
                  <div className="absolute top-2 right-2 bg-green-500 text-white text-[10px] md:text-xs font-semibold px-1.5 md:px-2 py-0.5 md:py-1 rounded-full flex items-center gap-1">
              ...
              {/* Info */}
              <div>
                <h3 className="text-white font-semibold text-xs sm:text-sm md:text-base line-clamp-1 group-hover:text-purple-400 transition-colors">
                  {module.name}
                </h3>
                <p className="text-gray-400 text-[10px] sm:text-xs mt-0.5 md:mt-1">
                  {module.course_lessons?.length || 0} aulas
                  {progress > 0 && progress < 100 && (
                    <span className="text-purple-400 ml-1 md:ml-2">{progress}%</span>
                  )}
                </p>
              </div>
            </div>
